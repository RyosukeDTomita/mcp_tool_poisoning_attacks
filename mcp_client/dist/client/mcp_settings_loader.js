...
function readMcpJson(mcpJsonPath) {
    ...
    try {
        ...
        throw new Error(`Failed to parse MCP JSON file: ${error instanceof Error ? error.message : "Unknown error"}`);
    }
}
/**
...
function getMcpParams(mcpJsonPath, serverName) {
    ...
    // どちらも定義されていない場合はエラー
    throw new Error(`Server '${serverName}' has invalid configuration: neither 'command' nor 'url' is defined`);
}
